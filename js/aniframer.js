function Aniframer(id_canvas, updateCallBack) {
    ...
    */
    ...
    this.checkResize = window.setInterval(function () {
        updateCanvasSize(_this, id_canvas);
    }, 500);
    ...
    function updateCanvasSize(_this, id_canvas) {
        ...
        _this.canvas.w = window.innerWidth;
        _this.canvas.h = window.innerHeight;
...
Aniframer.prototype.loop = function () {
    ...
    if (this.run) {
        ...
        window.requestAnimationFrame(function () {
            _this.loop();
        });	//Every newFrame this.loop() is called.
    }
...
Aniframer.prototype.addAudio = function (audio_name, audio_path, callBackFunction) {
    if (typeof audio_name === "string" && audio_name.length > 1 && typeof audio_path === "string" && audio_path.length > 4) {
        ...
        var _this = this;
        this.audio[audio_name].audio.addEventListener('canplaythrough', function () {
            preloadAudio(_this, audio_name, callBackFunction);
        }, false);
    }
};
...
/* Run/Stop Animation  */
/* ####################################################### */
Aniframer.prototype.startAnimation = function () {
    this.run = true;
    this.loop();
};
Aniframer.prototype.stopAnimation = function () {
    this.run = false;
};